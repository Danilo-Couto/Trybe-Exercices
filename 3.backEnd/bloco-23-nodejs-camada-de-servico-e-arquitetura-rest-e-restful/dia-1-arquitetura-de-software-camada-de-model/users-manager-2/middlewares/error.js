...
module.exports = (err, req, res, _next) => {
    ...
    if (err.status) {
        return res.status(err.stauts).json({ message_1: 'status', message: err.message });
    }

    // Caso o erro seja um erro do joi
    if (err.isJoi) {
        // Devolvemos o status 400 Bad Request com a mensagem de erro que o Joi gerou.
        return res.status(400).json({ message_2: 'details', message: err.details[0].message });
    }
    ...
    res.status(500).json({ message_3: 'Erro interno do servidor' });
};